...
public class SwipeRefreshLayout extends ViewGroup {
	...
	private final Animation mAnimationToRefreshPosition = new Animation() {
		@Override
		public void applyTransformation(float interpolatedTime, Transformation t) {
			mRefreshStartPosition = (int) (mOriginalOffsetTop
					- mRefreshIndicatorHeight + mDistanceToTriggerSync);
			int targetTop = 0;
	...
	private final Runnable mReturnToStartPosition = new Runnable() {
		...
		public void run() {
			...
			animateOffsetToStartPosition(mCurrentTargetOffsetTop
					- mRefreshIndicatorHeight, mReturnToStartPositionListener);
		}
	...
	private final Runnable mOffsetToRefreshPosition = new Runnable() {
		@Override
		public void run() {
			animateOffsetToRefreshPosition(mCurrentTargetOffsetTop
					- mRefreshIndicatorHeight, mGoingToRefreshPositionListener);
		}
	};
	...
	private final Runnable mCancel = new Runnable() {
		...
		public void run() {
			mReturningToStart = true;
			animateOffsetToStartPosition(mCurrentTargetOffsetTop
					+ getPaddingTop() - mRefreshIndicatorHeight,
		...
		case MotionEvent.ACTION_MOVE:
			if (mDownEvent != null && !mReturningToStart && !mRefreshing) {
				...
				if (yDiff > mTouchSlop || mPulling) {
					...
					float offsetTop = mCurrentTargetOffsetTop
							+ (eventY - mPrevY) / 2;
